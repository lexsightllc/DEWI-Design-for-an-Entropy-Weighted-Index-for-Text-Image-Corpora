...
class RedundancyEstimator:
    ...
    def compute_cross_modal_similarity(self, texts: List[str], images: List[Image.Image]):
        ...
        with torch.no_grad():
            tfeat = self.clip.get_text_features(input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"])
            if "pixel_values" not in inputs:
                raise ValueError("No image pixel_values provided.")
            ifeat = self.clip.get_image_features(pixel_values=inputs["pixel_values"])
